...
submitUserDotCom("test_email@test.com")
function submitUserDotCom(email){
  console.log(email)
  var data = JSON.stringify({
    "custom_id": email,
    "tags": ['Whatsapp Widget lead']
  });
...
function tmWidgetInit(widgetConfig) {
  ...
  var size = widgetConfig.size;
  var position = widgetConfig.position;
  var call = widgetConfig.call;
  var text = widgetConfig.text;
  var phone = validatePhone(widgetConfig.phone);
  var whatsappLink = "https://wa.me/" + phone + "/?text=" + text;
  var widgetElement = document.createElement("div");
  var headId = document.getElementsByTagName("head")[0];
  var link = document.createElement("link");
  link.type = "text/css";
  link.rel = "stylesheet";
  link.href = "https://d3kzab8jj16n2f.cloudfront.net/v1/widget-style.css";
  headId.appendChild(link);
  if (widgetConfig.type == "ww-extended") {
    htmlString = getExtendedHtmlString(whatsappLink, widgetConfig);
  } else {
    htmlString = getStandardHtmlString(whatsappLink, call);
  }

  widgetElement.id = "whatsapp-widget";
  widgetElement.classList.add(String(size));
  widgetElement.classList.add(String(position));
  widgetElement.classList.add(String(widgetConfig.type));
  widgetElement.innerHTML = htmlString.trim();
  document.body.appendChild(widgetElement);

  if (widgetConfig.type == "ww-extended") {
    document.getElementsByClassName("ww-extended")[0].addEventListener("click", function () {
      document.getElementsByClassName("ww-chat")[0].classList.toggle("display");
    });
    document.getElementsByClassName("ww-close")[0].addEventListener("click", function () {
      document.getElementsByClassName("ww-chat")[0].classList.remove("display");
    });
  }
}
...
function initAmplitude(widgetConfig) {
  ...
  var user_id_for_amplitude = window.location.hostname;
...
function getExtendedHtmlString(whatsappLink, config) {
  htmlString = `<div class="ww-icon">
    ...
    <div>
      <a rel="nofollow" class="ww-link" type="link" href="https://timelines.ai">TimelinesAI</a>
    </div>
  </div>
  <div class="ww-chat">
    <div class="ww-chat__wrapper" >
      ...
      <div class="ww-chat__body">
        <div class="ww-chat__message">
          ...
          <div class="ww-chat__message-body">${config.message}</div>
        </div>
      </div>
      <div class="ww-chat__button-container">
        <a class="ww-chat__button" role="button" href="${whatsappLink}" title="WhatsApp" rel="noreferrer noopener">
